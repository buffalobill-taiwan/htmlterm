...
export class MahjongAI {
    ...
    evaluateTargets(game, playerIdx, difficulty) {
        ...
        for (let v = 1; v <= 7; v++) {
            const k = 'honor' + v;
            const c = counts[k] || 0;
            const isYaku = (v >= 5) || (v === game.roundWind + 1) || (v === p.seatWind);
            if (isYaku) {
                if (c >= 2) yakuhaiCount += (c >= 3 ? 100 : 50);
                for (const m of melds) if ((m.type === 'triplet' || m.type === 'kan') && m.tiles[0].key() === k) yakuhaiCount += 100;
            }
        }
