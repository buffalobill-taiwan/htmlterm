...
export { ALL_TILE_TYPES };

export function getCounts(tiles) {
    const m = {};
    for (const t of tiles) m[t.key()] = (m[t.key()] || 0) + 1;
    return m;
}

export function removeTiles(tiles, key, n) {
    const r = [];
    let rem = n;
...
export function key(suit, value) {
    return suit + value;
}

export function findTile(tiles, suit, value) {
    for (const t of tiles) {
...
export function hasTile(counts, suit, value) {
    return (counts[key(suit, value)] || 0) > 0;
}

// ===== Dora =====

export function nextDoraTile(tile) {
...
export function decomposeMelds(tiles, numMelds) {
...
export function findDecompositionsWithOpen(tiles, openMelds) {
...
export function detectWaitType(hand, melds, pair, winTile, winType) {
    let winInMeld = null;
    let winIsPair = false;
...
export function detectWaitTypeSimple(hand, melds, pair, winTile) {
...
export function checkChiitoitsuDecomp(tiles) {
    const counts = getCounts(tiles);
...
export function checkKokushiDecomp(tiles) {
...
export function canFormCompleteHand(hand, openMelds, winTile) {
...
export function checkAllYaku(handInfo, gameState) {
    const yaku = [];
...
export function checkStandaloneYaku(handInfo, gameState) {
    const yaku = [];
...
export function calculateFu(handInfo, gameState) {
...
export function evaluateHand(hand, openMelds, winTile, winType, gameState) {
    let allTiles;
    if (winTile) {
...
export function isWinningHand(hand, openMelds) {
...
export function getWaitingTiles(hand, openMelds) {
    const waits = [];
...
export function checkTenpai(hand, openMelds) {
    return getWaitingTiles(hand, openMelds).length > 0;
}

export function getDoraCount(tiles, doraIndicators) {
